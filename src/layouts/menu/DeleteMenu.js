...
const DeleteMenu = ({deleteopen,setDeleteOpen}) => {
    ...
    useEffect(()=>{
        ...
        .then((res)=>{
            ...
            let rows=[]
            res.data?.map((item)=>(
                rows.push({"name" : item.menuName,"menuCode" : item.menuCode, 
  ...
  return (
        <div style={{ minHeight: "calc(100vh - 190px)",width:"100%" ,backgroundColor:textColor}}>
            ...
            <MDBox  sx={{display:"flex",justifyContent:"center"}} >
              ...
              {recovergridModal  && <RecoverMenuModel gridModal={recovergridModal} setGridModal={setRecoverGridModal} data={id} value={table}  />}
        </div>   
  )
}
