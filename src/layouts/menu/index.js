...
const Menu = () => {
                         ...
                         rows.push({
                              ...
                              view: (
                                   <MDTypography sx={{ fontSize: '1rem', fontWeight: '500' }}>
                                        {item.isActive == true ? (
                                             <MDTypography sx={{ fontSize: '1rem', fontWeight: '500', color: 'green' }}>
                                                  Active
                                             </MDTypography>
                                        ) : (
                                             <MDTypography sx={{ fontSize: '1rem', fontWeight: '500', color: 'red' }}>
                                                  Blocked
                                             </MDTypography>
                                        )}
                                   </MDTypography>
                              ...
                              edit: (
                                   <Icon
                                        style={{ cursor: 'pointer', marginLeft: '38%' }}
     ...
     const handleSubmit = () => {
          if (!menuname) {
               return toast.error(`Menu field required!`, {
                    position: 'top-right',
                    autoClose: 5000,
     ...
     const toggleRecover = (id) => {
          setId(id);
          setDeleteGridModal(!recovergridModal);
     };
     const reset = () => {
          setMenuname('');
     };
     ...
     return (
          <DashboardLayout>
               ...
               <div style={{ minHeight: 'calc(100vh - 190px)', width: '100%', backgroundColor: textColor }}>
                    {deleteopen ? (
                         <DeleteMenu deleteopen={deleteopen} setDeleteOpen={setDeleteOpen} />
                    ) : !open ? (
                         <>
                              <MDBox
                                   fullwidth='true'
                         ...
                         <>
                              <MDBox fullwidth='true'>
                                   ...
                                   <MDBox sx={{ paddingLeft: '55px', padding: '55px' }}>
                                        <MDTypography>Menu Name</MDTypography>
                                        <CustomInput width='300px' setValue={setMenuname} />
                                   </MDBox>
                    ...
                    {}
                    {gridModal && <MenuStatusModel gridModal={gridModal} setGridModal={setGridModal} data={id} value={dataFromApi} />}
                    {deletegridModal && (
                         <DeleteMenuStatus
                              gridModal={deletegridModal}
                              setGridModal={setDeleteGridModal}
                              data={id}
                              value={dataFromApi}
                         />
                    )}
